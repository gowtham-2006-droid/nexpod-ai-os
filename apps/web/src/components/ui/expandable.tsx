...
const ExpandableCard = React.forwardRef<HTMLDivElement, ExpandableCardProps>(
  (
    ...
    return (
      <>
        {/* Layout Placeholder Spacer to prevent collapse when card goes absolute */}
        {isExpanded && (
          <div
            style={{
              height: collapsedSize.height,
              width: expandDirection === "vertical" ? collapsedSize.width : undefined,
            }}
            className="w-full pointer-events-none opacity-0 shrink-0"
          />
        )}
        <motion.div
          ref={ref}
          className={cn(
            ...
            isExpanded ? "absolute z-40 top-0 left-0 shadow-2xl" : "relative w-full",
            className
          )}
          style={{
