...
export default function LandingPage() {
  ...
  const [messages, setMessages] = useState<{ id: string; text: string; sender: 'ai' | 'user' }[]>([
    {
      id: 'msg-1',
      text: 'Hello! I am your NexPod Operations Assistant. How can I help you manage your autonomous retail pod fleet today?',
      sender: 'ai',
    }
  ]);
  ...
  const handleSendMessage = (textToSend?: string) => {
    ...
    const newMessages = [
      ...messages,
      { id: userMsgId, text, sender: 'user' as const }
    ];
    ...
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary selection:text-white dark:selection:text-black">
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <AnimatePresence>
          {isChatOpen && (
            <motion.div
              ...
              className="mb-4 w-96 max-h-[520px] h-[520px] rounded-3xl border border-strokedark bg-boxdark shadow-2xl flex flex-col overflow-hidden text-left"
            >
              {/* Chat Header */}
              <div className="p-4 border-b border-strokedark bg-black/40 flex flex-col gap-1.5 shrink-0">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <div className="w-6 h-6 rounded-lg bg-chart-2 flex items-center justify-center">
                      <Brain className="w-3.5 h-3.5 text-black" />
                    </div>
                    <span className="text-xs font-bold text-white font-mono">Operations Assistant</span>
                  </div>
                  <button
                ...
                <Breadcrumb>
                  <BreadcrumbList className="text-[10px] font-mono text-slate-300">
                    <BreadcrumbItem>
                      <BreadcrumbLink className="cursor-pointer text-slate-400 hover:text-white">NexPod OS</BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator className="text-slate-500" />
                    <BreadcrumbItem>
                      <BreadcrumbLink className="cursor-pointer text-slate-400 hover:text-white">Support</BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator className="text-slate-500" />
                    <BreadcrumbItem>
                      <BreadcrumbPage className="text-white">Assistant</BreadcrumbPage>
                    </BreadcrumbItem>
                  </BreadcrumbList>
                </Breadcrumb>
              </div>
              ...
              <div className="flex-1 overflow-hidden flex flex-col relative">
                <MessageScroller className="flex-1">
                  <MessageScrollerViewport className="flex-grow p-4 no-scrollbar">
                    <MessageScrollerContent>
                      <BubbleGroup>
                        {messages.map((msg, index) => (
                          ...
                          >
                            ...
                            >
                              <BubbleContent className={msg.sender === 'user' ? 'bg-chart-2 text-black font-semibold' : 'bg-black/5 dark:bg-black/30 border border-strokedark text-black dark:text-white'}>
                                {msg.text}
                              </BubbleContent>
                            </Bubble>
                          </MessageScrollerItem>
                        ...
                        {isTyping && (
                          <MessageScrollerItem scrollAnchor>
                            <Bubble variant="secondary" align="start">
                              <BubbleContent className="bg-black/5 dark:bg-black/30 border border-strokedark text-bodydark2">
                                <span className="flex items-center gap-1.5">
                                  <span className="w-1.5 h-1.5 rounded-full bg-chart-2 animate-bounce" />
                                  <span className="w-1.5 h-1.5 rounded-full bg-chart-2 animate-bounce [animation-delay:0.2s]" />
                                  <span className="w-1.5 h-1.5 rounded-full bg-chart-2 animate-bounce [animation-delay:0.4s]" />
                                </span>
                              </BubbleContent>
                            </Bubble>
                          </MessageScrollerItem>
                      ...
                      <MessageScrollerItem>
                        <div className="pt-2">
                          <span className="text-[9px] font-mono text-bodydark2 uppercase tracking-wider block mb-1.5">Quick Questions</span>
                          <div className="flex flex-wrap gap-1.5">
                            {[
                              ...
                              <button
                                ...
                                className="px-2.5 py-1 rounded-lg border border-strokedark bg-black/40 hover:bg-chart-2/10 hover:border-chart-2/40 text-[10px] text-bodydark2 hover:text-white transition-all cursor-pointer font-mono"
                              >
                                {q}
                              </button>
                            ))}
                      ...
                      <MessageScrollerItem>
                        <div className="pt-4 border-t border-strokedark/50 mt-6 pb-2">
                          <span className="text-[10px] font-mono text-bodydark2 uppercase tracking-wider block mb-2">FAQ & Quick Info</span>
                          <Accordion className="space-y-1">
                            <AccordionItem value="item-1" className="border border-strokedark bg-black/20 rounded-xl px-3 py-1">
                              <AccordionTrigger className="text-xs text-black dark:text-white hover:text-primary py-2 cursor-pointer">
                                How does stock optimization work?
                              </AccordionTrigger>
                              <AccordionContent className="text-[11px] text-bodydark2 pb-2 leading-relaxed">
                                The AI reads telemetry parameters like consumption velocity and reorder thresholds, forecasting refill times before stock runs empty.
                              </AccordionContent>
                            </AccordionItem>
                            
                            <AccordionItem value="item-2" className="border border-strokedark bg-black/20 rounded-xl px-3 py-1">
                              <AccordionTrigger className="text-xs text-black dark:text-white hover:text-primary py-2 cursor-pointer">
                                Can it bind to physical IoT devices?
                              </AccordionTrigger>
                              <AccordionContent className="text-[11px] text-bodydark2 pb-2 leading-relaxed">
                                Yes, the APIs and data models are designed to bind directly to hardware components (e.g. ESP32, sensor loops) via HTTP/REST seams.
                              </AccordionContent>
                            </AccordionItem>
                          </Accordion>
                        </div>
                  ...
                  <MessageScrollerButton direction="end" className="text-black bg-chart-2 hover:bg-chart-2/95 border-none cursor-pointer" />
                </MessageScroller>
              </div>
              ...
              <form
                ...
                className="p-3 border-t border-strokedark bg-black/40 flex gap-2 shrink-0"
              >
                <input
                  type="text"
                  value={inputValue}
                  onChange={(e) => setInputValue(e.target.value)}
                  placeholder="Ask about NexPod AI OS..."
                  className="flex-grow bg-black/50 border border-strokedark rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-primary font-mono"
                />
                <button
                  type="submit"
                  className="p-2 rounded-xl bg-chart-2 text-black hover:bg-chart-2/80 transition-colors cursor-pointer"
                >
        ...
        <motion.button
          ...
          className="flex h-12 w-12 items-center justify-center rounded-full bg-primary text-black shadow-xl hover:bg-primary/95 transition-colors cursor-pointer"
        >
          {isChatOpen ? <X className="w-5 h-5" /> : <Brain className="w-5 h-5" />}
        </motion.button>
      </div>
    </div>
  );
